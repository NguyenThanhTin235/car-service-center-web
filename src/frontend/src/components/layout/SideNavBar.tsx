    ...
    <aside className="fixed top-0 left-0 h-screen w-64 z-40 bg-surface-container-lowest dark:bg-inverse-surface border-r border-outline-variant dark:border-outline shadow-sm dark:shadow-none flex flex-col justify-between">
      ...
      <div className="flex flex-col h-full overflow-y-auto">
        ...
        <nav className="flex-1 px-3 py-2 space-y-1">
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && !item.badge && <span className="w-1.5 h-1.5 rounded-full bg-on-primary"></span>}
              </Link>
            );
          })}
          ...
          <Link
            href="#"
            className="flex items-center gap-3 px-3 py-2 rounded-lg text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface text-body-md font-body-md transition-colors"
          >
